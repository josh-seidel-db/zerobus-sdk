(** Phase 7b façade acceptance: the PUBLIC {!Zerobus} API auto-selects the Arrow
    ...
    from the promoted {!Zerobus_arrow} codec, against the Arrow/Flight mock server.
       ...
       | Ok stream ->
           ...
           let rec loop i last =
             if i >= n_records then Lwt.return (Ok last)
               ...
               with
               | Error e -> Lwt.return (Error (Zerobus_core.Error.Stream_error ("arrow encode: " ^ e)))
               | Ok ipc ->
           ...
           (match ingested with
            ...
            | Ok last_off ->
                ...
                Lwt.return { acked_last; issued = n_records;
  ...
  lazy
    (Lwt_main.run
       (with_server (fun () ->
            let* r = run () in
            Printf.eprintf
              "ZEROBUS OCAML PHASE 7b — PUBLIC FACADE auto-selects Arrow/Flight -- evidence\n\
               ocaml_version : %s\n\
               entry         : Zerobus.create_stream_with_headers (record_type=Arrow)\n\
