(** Graceful-close acceptance: the driver's [stream_paused_max_wait_time_ms] path
    end to end over Lwt, against a mock that sends a [CloseStreamSignal] mid-way on
    the first stream and then ends the body cleanly (NOT a retryable error status).

    With the default [stream_paused_max_wait_time_ms = None] and recovery on, the
    driver must: keep draining the acks the server already sent, observe the clean
    body end, then reconnect+replay the un-acked tail — so [flush] still confirms all
    200 records durably acked, with the server-initiated close invisible above
    [ingest]/[flush]. This is the regression test for the graceful-close bugs the
    code review caught (sleep-blocks-draining, unbounded reconnect, waiter hang). *)
...
let result = lazy (Lwt_main.run (with_server (run_bounded ?paused_max_wait_ms:None)))
  ...
  Alcotest.run "graceful-close"
    [
      ( "drain-then-recover (None = full server window)",
        [
          Alcotest.test_case "no fatal error (recovered across close)" `Slow (fun () ->
              let r = Lazy.force result in
              Alcotest.(check (option string)) "err" None r.err);
          Alcotest.test_case "flush confirms all acked across the close" `Slow (fun () ->
              let r = Lazy.force result in
              Alcotest.(check bool) "flushed" true r.flushed_ok);
          Alcotest.test_case "acks observed (watermark advanced, no hang)" `Slow (fun () ->
              let r = Lazy.force result in
              Alcotest.(check bool) "acks_seen > 0" true (r.acks_seen > 0));
        ] );
      ( "finite cap (Some 300ms) fires before a held-open body",
        [
          Alcotest.test_case "no fatal error (capped then recovered)" `Slow (fun () ->
              let r = Lazy.force result_capped in
              Alcotest.(check (option string)) "err" None r.err);
          Alcotest.test_case "flush succeeds via the cap (not the 10s body)" `Slow
            (fun () ->
              let r = Lazy.force result_capped in
              Alcotest.(check bool) "flushed" true r.flushed_ok);
        ] );
