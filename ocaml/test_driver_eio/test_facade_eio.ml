(** Public-API acceptance on Eio: drives the ergonomic {!Zerobus_eio} façade
    ...
    [EphemeralStream] server (separate process, cleartext h2c). Counterpart of the
    raw-driver [test_driver_eio]; proves the public bracket-shaped surface.

    Runs on zbeio (OCaml 5.2). Auth is caller-supplied headers (no built-in OAuth
    on Eio — see the module doc). *)
...
let run ~env : result =
  ...
  | Ok client ->
      ...
      in
      (match Zb.with_stream ~env ~sw ~tls:false client table ~headers ~options body with
       | Ok r -> r
       | Error e -> { acked_last = false; issued = 0; err = Some (Zerobus_core.Error.to_string e) })
