(** Eio built-in OAuth acceptance: exercises {!Zerobus_eio.mint_token}'s full
    ...
    parse the JSON [access_token] — against a local cohttp-eio mock token endpoint.
    ...
    carried [grant_type=client_credentials] + the Basic auth header, and a non-2xx
    surfaces as [Auth_error]. Runs on zbeio. *)
...
let start_server ~sw ~env : unit =
  ...
  let handler _conn request body =
    ...
    captured_auth :=
      Option.value (Http.Header.get (Http.Request.headers request) "authorization")
        ~default:"";
...
let run ~env ~sw : result =
  ...
  with
  | Error e -> { token = None; err = Some (Zerobus_core.Error.to_string e); body_has_grant = false; auth_is_basic = false }
  | Ok client ->
      let res =
      ...
      (match res with
       | Error e -> { token = None; err = Some (Zerobus_core.Error.to_string e); body_has_grant; auth_is_basic }
       | Ok token -> { token = Some token; err = None; body_has_grant; auth_is_basic })
      ...
      ( "client-credentials mint",
        [
          Alcotest.test_case "token returned" `Slow (fun () ->
              ...
              Alcotest.(check bool) "got token" true (r.token = Some "tok-abc123"));
