(** Live-TLS acceptance on Async: drives the {!Zerobus_async} façade over a REAL
    ...
    server prints its cert SHA-256 (base64) on the READY line and the client
    pins it via {!Zerobus_async.Io_async_for_test}'s
    [Tls_connect.pinned_cert_fp_sha256_b64] — the Async equivalent of the Eio
    [~authenticator] override.

    Two cases:
    - {b positive}: with the correct pin, 200 records are issued and [flush]
      confirms they were all acked over TLS.
    - {b negative}: with a WRONG pin, the handshake's peer-cert verification
      must fail, so stream creation returns an error (no records acked). This is
      what proves we do real certificate verification (not a null
      authenticator).
      ...
      ( "live-tls",
        [
          ...
          Alcotest.test_case "flush confirms all acked over TLS" `Slow
            (fun () -> Alcotest.(check bool) "acked" true positive.acked_last);
          Alcotest.test_case "wrong cert pin is rejected" `Slow (fun () ->
              Alcotest.(check bool)
                "no ack on bad cert" false negative.acked_last;
              Alcotest.(check bool)
                "error present on bad cert" true
