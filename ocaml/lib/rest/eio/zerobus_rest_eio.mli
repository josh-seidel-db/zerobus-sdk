(** The stateless REST interface for the Zerobus Ingest SDK — Eio runtime
    ...
    The Eio (direct-style) counterpart of {!Zerobus_rest} (the Lwt reference): a
    thin, optional helper for low-frequency / edge cases where the persistent
    gRPC stream ({!Zerobus_eio}) is overkill. One HTTP [POST] per batch, no
