...
(** Parse workspace-id from a Databricks workspace URL.
    ...
    Handles standard Databricks URL formats across Azure, AWS, and GCP.
...
(** Derive gRPC endpoint (host, port) from workspace metadata.
    ...
    If [endpoint] is provided (non-empty, not "default"), it is parsed and returned
    as-is. Otherwise, the endpoint is derived from [workspace_url].
...
(** Build the OAuth 2.0 client-credentials token request body.
    ...
    Returns [Error.Auth_error] if the table is not in "catalog.schema.table" format.
*)
