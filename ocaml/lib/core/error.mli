(** The SDK error taxonomy. Mirrors the sister SDK's [Error.t] shape and the Rust
    core's retryable/non-retryable classification (see rust `is_retryable`), so
    recovery (§12.3) can decide what to retry. Results-over-exceptions is a golden
    rule: public operations return [(_, Error.t) result], they do not raise. *)
...
(** Whether an operation that failed with this error is worth retrying —
    the recovery driver consults this. Transport errors, timeouts, and the
    retryable gRPC status codes (UNAVAILABLE=14, etc.) are retryable; auth,
    protocol, and stream-misuse errors are not. *)
