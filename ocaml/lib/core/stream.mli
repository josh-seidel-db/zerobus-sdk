(** The runtime-agnostic streaming driver (DESIGN.md §5, §6.4), functorized over
    ...
    The driver's offset/ack/watermark/recovery control plane is
    protocol-agnostic; the on-the-wire specifics live behind {!PROTOCOL}. [Make]
    uses the default {!Ephemeral} (Zerobus [EphemeralStream], JSON/Proto). An
    Arrow/Flight [DoPut] driver is obtained by applying {!Make_with_protocol} to
    a Flight [PROTOCOL] defined OUTSIDE [lib/core] (so core needs no Flight
    proto / libarrow). *)
