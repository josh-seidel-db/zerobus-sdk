(** Abstract bidirectional gRPC transport.
    ...
    The Zerobus hot path is a single client-initiated bidirectional streaming
    RPC (`EphemeralStream`, and Arrow's `DoPut`): the client pushes framed
    messages while concurrently draining acknowledgements. The three runtime
    spikes proved this shape on `grpc-lwt` (push-fn + [Lwt_stream]),
    `grpc-async` ([Pipe]), and `grpc-eio` ([Seq]). Despite the surface
    differences, all three reduce to the same three operations, which this
    signature captures:
    ...
    moves bytes. This is what {!Io.IO.H2_client} is required to provide, and
    what the streaming driver ({!Zerobus_core.Make}) runs its send-fiber /
    ack-daemon pair over. Keeping it a bare signature means [zerobus-core] links
    no runtime. *)
...
module type S = sig
  ...
  *)
  ...
  (** Establish a connection to [host]:[port]. When [tls] is [true] (the
      default) the runtime packages negotiate TLS 1.3 + ALPN [h2] (proven in
      [spike-live/]) and fail fast if ALPN ≠ [h2]; when [false] they use
      cleartext h2c (the loopback mock backends). [tls] is an explicit flag
      rather than a port heuristic so a bearer token is never sent over
      cleartext by accident. [headers] are connection-level metadata (e.g. the
      table-name header). *)
  ...
  (** Queue one framed message on the request side. Returns once buffered; the
      ...
      finding, §6.5 — the Eio instantiation must ensure a flush actually
      occurs). *)
