(** The minimal effect surface the SDK is functorized over (DESIGN.md §6.2).
    ...
    The whole SDK is [Make(Io : IO)]; three thin packages instantiate it for
    Lwt, Eio, and Async. The exact shape is driven by two correctness findings
    (design review #1/#2), both stemming from the core being *concurrent
    bidirectional streaming* rather than request/response:
    ...
    - {b Concurrency combinators take thunks, not values.} In a direct-style
      ...
      already run — sequentially. [both]/[fork_daemon] therefore take a
      suspended [unit -> _] so the {e runtime}, not the call site, decides when
      each side runs. (Harmless on Lwt/Async, where ['a t] is already
      suspended.)
...
module type IO = sig
  ...
  *)
  ...
  type 'a io = 'a t
  (** Alias for the effect type, so nested signatures (e.g. [Scope]) can refer
      to it without [t] being shadowed by a local [type t]. *)
  ...
  module Scope : sig
    ...
    (** Run [f] with a fresh scope. When [f] returns or raises, every daemon
        forked into the scope (via {!fork_daemon}) is cancelled and joined
        before [with_scope] returns. This is the stream's lifetime boundary:
  ...
  (** Run two computations concurrently and return the result of whichever
      finishes FIRST, abandoning (Lwt/Eio: cancelling) the loser. Both branches
      yield the same type so the caller can race a real computation against a
      timer branch (e.g. [first connect (fun () -> sleep t; Timeout)]) — see the
      [timeout] use in the stream driver. Cancellation semantics follow the
      runtime: Lwt [Lwt.pick] and Eio [Fiber.first] hard-cancel the loser; Async
      has no hard fiber cancellation, so the losing [Deferred] is simply
      detached (matching the discipline in {!Scope}) — callers must therefore
      keep loser side effects idempotent / harmless when abandoned. *)
