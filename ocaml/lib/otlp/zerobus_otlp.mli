(** The OTLP (OpenTelemetry) exporter for the Zerobus Ingest SDK (DESIGN.md §4.3).
    ...
    (reusing [Zerobus]'s Lwt [H2_client]).

    A unary Export is a degenerate bidi call: send one framed request, half-close,
    read one framed response, then the gRPC status. Unlike {!Zerobus} there is no
    stream, no offset, no recovery — each [export_*] is one request/response.

    The OTLP wire types (the OpenTelemetry collector protos, vendored
    independently of the Zerobus protos) are in the [Zerobus_otlp_proto] library:
    build a [Logs.resource_logs list] / [Metrics.resource_metrics list] with its
    generated [make_*] builders and hand them to {!export_logs} / {!export_metrics}.
...
(** Construct an OTLP exporter.
    ...
    - [endpoint]: the Zerobus gRPC/OTLP endpoint ([host] or [host:port]). If empty
      or ["default"], it is derived from [workspace_url] (same host derivation as
      the gRPC endpoint; port defaults to 443).
    - [workspace_url]: the Databricks workspace URL; used for the OAuth [resource]
      and endpoint derivation.
    ...
    - [tls]: negotiate TLS 1.3 + ALPN-h2 (default [true]); [false] uses cleartext
      h2c (loopback mock only — a bearer token is never sent over cleartext).
