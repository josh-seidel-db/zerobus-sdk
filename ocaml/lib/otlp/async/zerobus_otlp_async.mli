(** The OTLP (OpenTelemetry) exporter for the Zerobus Ingest SDK — Async runtime
    ...
    exporter for callers already emitting OpenTelemetry logs and metrics. It ships
    them to the Zerobus OTLP endpoint via the two unary RPCs
      - [opentelemetry.proto.collector.logs.v1.LogsService/Export]
      - [opentelemetry.proto.collector.metrics.v1.MetricsService/Export]
    over the {b same} TLS 1.3 + ALPN-h2 transport the gRPC streaming SDK uses
    (reusing {!Zerobus_async}'s Async [H2_client]).

    A unary Export is a degenerate bidi call: send one framed request, half-close,
    read one framed response, then the gRPC status. Unlike {!Zerobus_async} there
    is no stream, no offset, no recovery — each [export_*] is one
    request/response.

    The OTLP wire types come from the shared [Zerobus_otlp_proto] library (the same
    vendored OpenTelemetry collector protos the Lwt exporter uses).

    Auth reuses the same table-scoped client-credentials grant as {!Zerobus_async}
