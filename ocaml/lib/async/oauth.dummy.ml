(** Fallback OAuth backend for the Async runtime — used when [cohttp-async] is
    NOT installed (dune [select] picks this over [oauth.real.ml]). Built-in
    ...
    [headers_provider]. Signature parity with [oauth.real.ml]. To enable
    built-in OAuth, install [cohttp-async] on the switch — dune then selects the
    real backend automatically. See doc/arch/tls_async_status.md. *)
