(** Fallback TLS connect for the Async transport — used when [tls-async] is NOT
    ...
    TLS then returns an honest error; cleartext h2c (mocks) is unaffected. The
    Lwt and Eio runtimes are the live-TLS references. To enable real Async TLS,
    install [tls-async] on the switch (see doc/arch/tls_async_status.md) — dune
    then selects [tls_connect.real.ml] automatically. *)
  ...
  return
    (Error
       "Async TLS not available: install tls-async on this switch to enable it \
        (dune selects the real backend); use tls:false for a cleartext mock, \
        or the Lwt/Eio runtime for live TLS (see doc/arch/tls_async_status.md)")
