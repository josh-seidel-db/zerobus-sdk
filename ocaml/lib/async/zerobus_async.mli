(** The public Async API for the Zerobus Ingest SDK (DESIGN.md §5.2, §5.3).
    ...
    [with_stream_oauth]) rather than detached, matching Async's lack of hard
    fiber cancellation. Live TLS and built-in client-credentials OAuth are both
    available as dune [select]s on optional deps ([tls-async] and
    [cohttp-async]): when the dep is present they work; when absent,
    [with_stream_oauth]/[mint_token] return an [Auth_error] ([oauth_available]
    reports which) and TLS connects return an honest error — so the caller
    supplies a bearer via [headers_provider] and, with [~tls:false], drives the
    full façade→driver path against a cleartext mock. (Lwt/Eio are the always-on
    TLS references; see doc/arch/tls_async_status.md.)
...
(** Queue one record and return its offset immediately. Does NOT wait for the
    ack — the record is sent on a background job. *)
...
(** Wait once for all pending records to be durable — the end-of-loop call in
    the loop-then-flush pattern. *)
...
(** Close the stream and tear down its scope (best-effort join of the
    ack-reader). Normally handled by the {!with_stream} bracket; exposed for
    explicit close. *)
...
(** Bracket form: open a stream with caller-supplied gRPC headers (custom auth),
    ...
    endpoint if the provider omits it. [tls] defaults to [true]; pass [false]
    for a cleartext-h2c mock backend. Returns the result of [f], or [Error] if
    the stream could not be opened. *)

(** {1 Built-in client-credentials OAuth (§12.2)} *)

val oauth_available : bool
(** Whether built-in OAuth is compiled in — [true] iff [cohttp-async] was
    present at build time (a dune [select]). When [false], {!mint_token} /
...
(** Mint a table-scoped OAuth token via the client-credentials grant (HTTPS POST
    ...
    token is cached on [client] with its expiry and refreshed ~30s early.
...
(** The TLS connect backend (a dune [select] on tls-async: the real backend when
    ...
    live-TLS test can pin a self-signed cert via [pinned_cert_fp_sha256_b64] —
    the Async analogue of the Eio [~authenticator] override. Not part of the
    ergonomic surface (see doc/arch/tls_async_status.md). *)
module Tls_connect : sig
  val available : bool

  val pinned_cert_fp_sha256_b64 : string option ref
  (** When [Some "<sha256-b64>"], authenticate the TLS peer by pinning that
      exact certificate fingerprint instead of the system trust store — for
      testing against a self-signed h2 mock. Prod leaves it [None]. Inert in the
      honest-error stub backend. *)
end

(** The built-in OAuth HTTP backend (a dune [select] on tls-async). Exposed so
    the sibling REST/OTLP Async packages reuse the same pure-ocaml-tls client-
    credentials token POST (hand-rolled HTTP/1.1 over [Tls_async.connect])
    rather than cohttp-async's OpenSSL conduit. Not part of the ergonomic
    surface. *)
module Oauth : sig
  val available : bool
  ...
  val post_token :
    ...
    body:string ->
    (int * string, exn) result Deferred.t
  (** POST the client-credentials grant to [token_url] ([body] is the
      URL-encoded request body; [client_id]/[client_secret] go in the HTTP Basic
      header). Returns [(http_status, response_body)] or the request exception.
  *)

  val parse_token_response : now:float -> string -> (string * float) option
  (** Parse the token endpoint's JSON reply into
      [(access_token, expiry_epoch_s)] ([now] is the base for the absolute
      expiry); [None] if malformed. *)
end
