(** The public Eio API for the Zerobus Ingest SDK (DESIGN.md §5.2, §6.2).
    ...
    façade is bracket-shaped: {!with_stream} / {!with_stream_oauth} own the
    switch for the stream's lifetime and tear everything down on exit.
...
val ingest_records :
  stream -> bytes list -> (offset, Zerobus_core.Error.t) result
...
(** Open a stream with caller-supplied gRPC [headers] and run [f] with it,
    ...
    to [true]; pass [false] for a cleartext-h2c mock. [:authority] is derived
    from the endpoint if [headers] omit it. [authenticator] overrides the system
    trust store for the TLS handshake (default: [Ca_certs]) — mainly for tests
    pinning a self-signed cert. Returns the result of [f], or [Error] if the
...
(** Mint a table-scoped OAuth token via the client-credentials grant (HTTPS POST
    ...
    id/secret ride in the HTTP Basic header). Direct-style, over cohttp-eio +
    real TLS. Exposed for callers that want the token itself. *)
...
(** Open a stream with built-in client-credentials OAuth and run [f] with it
    (bracket form) — the ergonomic counterpart of the Lwt
    [Zerobus.create_stream]. Mints a table-scoped token, builds the bearer +
    table-name headers, opens the stream over TLS, runs [f], and tears down on
    exit. *)
