(** Public-API acceptance on Async: drives the ergonomic {!Zerobus_async} façade
    (create → create_stream_with_headers → ingest/flush/close) end to end
    against the mock [EphemeralStream] server (separate process, cleartext h2c).
    This is the counterpart of the raw-driver [test_driver_async]; it proves the
    public surface, not just the internal driver.
