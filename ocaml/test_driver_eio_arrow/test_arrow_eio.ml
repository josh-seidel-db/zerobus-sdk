(** Eio Arrow/Flight acceptance: the PUBLIC {!Zerobus_eio} façade auto-selects the
    Arrow Flight [DoPut] driver when [record_type = Arrow], with REAL Arrow IPC
    bodies from {!Zerobus_arrow}, against the Eio Arrow/Flight mock server. The Eio
    counterpart of test_driver_arrow (Lwt) and test_driver_async_arrow — closing the
    gap where Eio had no offline Arrow test.
          ...
          (fun stream ->
            let rec loop i last =
              if i >= n_records then Ok last
                ...
                with
                | Error e -> Error (Zerobus_core.Error.Stream_error ("arrow encode: " ^ e))
                | Ok ipc -> (
...
let result =
  lazy
    (with_server (fun port ->
         ...
         Printf.eprintf
           "ZEROBUS OCAML — EIO PUBLIC FACADE auto-selects Arrow/Flight -- evidence\n\
      ...
      ( "public facade auto-selects arrow/flight",
        [
          Alcotest.test_case "no error" `Slow (fun () ->
              Alcotest.(check (option string)) "err" None (Lazy.force result).err);
